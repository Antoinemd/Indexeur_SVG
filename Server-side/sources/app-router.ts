...
import { findAllIamgestWithShape, findAllImagesWithColor, findAllImagesWithBoth,
         PopulateBDD, flushBDD } from './mongoRequest';
...
export function getImagesRestApi(): express.Router {
    ...
    router.post("/findImages", (req, res) => {
        ...
        if (forme !== "none" && (couleur === undefined || couleur === "")) {
            ...
            sendResultat = findAllIamgestWithShape(req.body.forme);
        }
        // requete seulement pour les formes et les couleurs
        if (forme === "none" && (couleur !== undefined || couleur !== "")) {
            console.log("requete pour les formes et/ou les couleurs");
            sendResultat = findAllImagesWithBoth(req.body.forme, req.body.color);
        }
